...
interface SongService {
    ...
    @FormUrlEncoded
    @POST("like?cookie=${MyApplication.fakeCookie}")    //喜欢音乐 like 默认ture,ture 为喜欢，false 为不喜欢
    fun likeSong(@Field("id") id: Int,@Field("like") like: Boolean):  Call<Any>

    @GET("playlist/tracks?cookie=${MyApplication.fakeCookie}")    //对歌单添加或删除歌曲 op: 从歌单增加单曲为 add, 删除为 del   pid: 歌单 id  tracks: 歌曲 id,可多个,用逗号隔开
    ...
    @FormUrlEncoded
    @POST("record/recent/song?cookie=${MyApplication.fakeCookie}")      //最近播放-歌单
    fun getRecentSongs(@Field("timestamp") timestamp:Long): Call<RecentPlayResponse>

    @FormUrlEncoded
    @POST("recommend/songs?cookie=${MyApplication.fakeCookie}")
